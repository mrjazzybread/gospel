...
let rec check ~verbose ~compile tasts env = function
  | [] -> tasts
  | file :: t ->
      ...
      let tasts, mods =
        if Filename.extension file = gospel_ext then
          ...
          let tast, mods = check_file ~verbose ~comp_dir ~compile ~env file in
          if verbose then (
            pp fmt "@[@\n*******************************@]@.";
            pp fmt "@[********** Typed GOSPEL *******@]@.";
            pp fmt "@[*******************************@]@.";
          ...
          (file :: tasts, mods)
      in
      check ~verbose ~compile tasts (Namespace.add_mod env id mods) t

let tast ~verbose ~compile files =
  if compile then print_endline "what";
  errors files;
  (* Create the compilation directory if it does not already exist. *)
  if compile && not (Sys.file_exists comp_dir) then
    (* The integer argument [0o777] creates the directory with full
       permissions. *)
    Sys.mkdir comp_dir 0o777
  else if compile && not (Sys.is_directory comp_dir) then
    error
  ...
  check ~verbose ~compile [] env files
