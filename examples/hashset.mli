...
module type HashedType = sig
  ...
  (*@ function h (x : t) : int *)
  (*@ axiom h_equal : forall x y, equiv x y -> h x = h y *)
...
module type SET = sig
  ...
  (*@ predicate mem_equiv (x : element) (s : element set) =
  ...
  (*@ function eq_class (x : element) : element set =
       {y | equiv x y} *)
...
module type MAP = sig
  ...
  (*@ function present (x : key) (m : (key * value) set) =
        exists v, mem (x, v) m *)
