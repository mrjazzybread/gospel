...
(* The following functions make use of "deep types". These are a
   ...
   [deep] function which returns a variable whose type must be equal
   to the provided deep type *)

(** Deep type for booleans *)
...
(** [map_binders f xs] receives a function that maps values of type ['a] to
    binders that introduce variables of type ['b] and a list of values of type
    ['a]. This function returns a binder for a list of ['b]. *)
let rec map_binders (f : 'a -> ('b, 'r) binder) (xs : 'a list) :
 ...
 fun k ->
  ...
  | x :: xs ->
      let@ y = f x in
      let@ ys = map_binders f xs in
      k (y :: ys)

(** [bind_variables l] creates a binder of a list where every element of [l] is
    associated with an Inferno variable. *)
...
let rec hastype (t : IdUast.term) (r : variable) =
  ...
  let+ t_node =
    ...
    | Tfalse ->
        ...
        Tfalse
    | Tconst constant ->
        (* Depending on the type of constant, we restrict the
    ...
    | Tapply (t1, t2) ->
        ...
        let@ arg_ty = exist in
        let+ t2 = hastype t2 arg_ty
        (* The term [t1] must be of the function type [arg_ty -> r]. *)
        and+ t1 = lift hastype t1 (S.ty_arrow arg_ty r) in
        Tapply (t1, t2)
    | Tinfix _ -> assert false
    (* This node is replaced by [Tapply] during the pre processing *)
    | Tquant (q, l, t) ->
        (* forall. x y z. t *)
...
let function_cstr (f : IdUast.function_) : Tast2.function_ co =
...
(** [module_cstr] creates a constraint whose semantic value is a list of
    ...
    same scope as the ones defined in [fun_types].*)
let rec module_cstr (m : IdUast.s_module_declaration) =
...
(** [signature ts cstr s] Creates a constraint that transforms the signature [s]
    into typed signature. *)
and signature_item s =
  let+ sdesc =
    ...
    | Sig_module m ->
        ...
        (* Since we handled modules and namespaces in [Inferno_prep], we don't
...
let signatures l =
  let l = Inferno_prep.signatures l in
