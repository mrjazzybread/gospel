...
and term_node ~loc env prop dty dterm_node =
  ...
  | DTapp (ls, [ dt1 ]) when ls.ls_field ->
      t_field (term env false dt1) ls (Option.fold ~some:ty_of_dty ~none:ty_bool dty) loc
  | DTapp (ls, dtl) ->
      t_app ls (List.map (term env false) dtl) (Option.fold ~some:ty_of_dty ~none:ty_bool dty) loc
  | DTif (dt1, dt2, dt3) ->
