...
let lookup f (defs : mod_defs) pid =
  let env = f defs in
  let str = pid.Identifier.Preid.pid_str in
  Env.find str env
...
let add_fun f defs =
  ...
  { defs with fun_env = Env.add f.Ident.id_str f env }
...
let add_mod m mod_defs defs =
  ...
  {
    defs with
    mod_env = Env.add m.Ident.id_str m menv;
    nested_defs = Env.add m.Ident.id_str mod_defs denv;
  }
...
let unique_pty defs env =
  ...
  and unique_label = function
    | ParseUast.Lnone id when id.pid_str = "_" -> Lnone (Ident.from_preid id)
    | _ -> assert false
  in
  unique_pty
...
let rec signature s env =
  ...
  let sdesc, env =
    ...
    | Sig_module m ->
        ...
        let id = Option.map Ident.from_preid m.mdname in
