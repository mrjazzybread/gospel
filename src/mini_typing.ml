...
module S = struct
  type 'a structure = Int | Bool | List of 'a

  let ty_bool = Bool
  let ty_int = Int
  let ty_list ty = List ty
  let iter f t = match t with List x -> f x | _ -> ()
  let fold f t accu = match t with List x -> f x accu | _ -> accu
  let map f t = match t with List x -> List (f x) | Int -> Int | Bool -> Bool
...
module O = struct
  ...
  type ty = Var of string | Int | Bool | List of ty
  ...
  let structure t =
    match t with S.Int -> Int | S.Bool -> Bool | S.List v -> List v
...
let rec leaf id = match id with Qpreid id -> id | Qdot (q, _) -> leaf q
...
let rec hastype (t : Uast.term) (w : variable) =
  ...
  | Uast.Tpreid q ->
      if (leaf q).pid_str = "empty" then
        let@ v = exist in
        let+ () = w --- S.ty_list v and+ v = decode w in
        mk_term Tnil (O.ty_list v)
      else
        let+ _ = instance q w and+ ty = decode w in
...
let typecheck t =
  let _, t =
    try Solver.solve ~rectypes:false (let0 (exist (hastype t)))
    with Solver.Unbound (_, t) ->
