...
let unsome = function None -> assert false | Some v -> v
let list_of_option = function None -> [] | Some v -> [ v ]
let option_app d f = function None -> d | Some x -> f x
let unsome_safe d = function None -> d | Some s -> s
let bool_of_option xo = match xo with None -> false | Some x -> x
...
let rec list_make n v = if n = 0 then [] else v :: list_make (n - 1) v
...
let list_nat n =
  (* for n >= 0 *)
  let rec aux i = if i = 0 then [] else (i - 1) :: aux (i - 1) in
  List.rev (aux n)
...
let list_unique l =
  let rec aux acc = function
    | [] -> acc
    | a :: q -> if List.mem a acc then aux acc q else aux (a :: acc) q
  in
  aux [] l

let list_intersect l1 l2 = List.filter (fun x -> List.mem x l1) l2
let list_minus l1 l2 = List.filter (fun t -> not (List.mem t l2)) l1
let list_concat_map f l = List.concat (List.map f l)
let list_assoc_option x l = try Some (List.assoc x l) with Not_found -> None

let rec assoc_list_map f = function
  | [] -> []
  | (k, v) :: l -> (k, f v) :: assoc_list_map f l

let rec list_remove i l =
  (* i >= 0 *)
  match l with
  | [] -> failwith "list_remove invalid index" (* todo: illegal argument *)
  | x :: t -> if i = 0 then t else x :: list_remove (i - 1) t

let rec list_replace i v l =
  (* i >= 0 *)
  match l with
  | [] -> failwith "list_replace invalid index" (* todo: illegal argument *)
  | x :: t -> if i = 0 then v :: t else x :: list_replace (i - 1) v t

let list_replace_nth i vs xs = list_replace i (List.nth vs i) xs
let list_ksort cmp l = List.sort (fun (k1, _) (k2, _) -> cmp k1 k2) l
...
(** [list_is_included l1 l2] returns true if any item in [l1] also belongs to
    [l2] *)

let list_is_included l1 l2 = List.for_all (fun x -> List.mem x l2) l1
...
let lin1 = "\n"
let lin2 = "\n\n"
let show_list s sep l = String.concat sep (List.map s l)

let show_listp s sep l =
  if l = [] then "" else sep ^ String.concat sep (List.map s l)

let show_listq s sep l =
  if l = [] then "" else String.concat sep (List.map s l) ^ sep

let show_option f ox = match ox with None -> "" | Some x -> f x
let show_par required s = if required then "(" ^ s ^ ")" else s
let show_str s = s
...
let unsupported_noloc s = failwith ("Unsupported language construction : " ^ s)
let unsupported _ s = unsupported_noloc s
let warning _ s = Format.fprintf Format.err_formatter "Warning: @[<v 2>%s@]\n" s

exception Not_in_normal_form of Location.t * string

let not_in_normal_form loc s = raise (Not_in_normal_form (loc, s))
