...
struct
  ...
  let map_ocaml_qid = function
    ...
    | Qid id -> Qid (map_ocaml_id id)

  let rec map_ocaml_ty = function
    | PTtyvar _ as t -> t
    | PTtyapp (info, l) ->
        PTtyapp
          ( { info with app_qid = map_ocaml_qid info.app_qid },
            List.map map_ocaml_ty l )
    | PTtuple l -> PTtuple (List.map map_ocaml_ty l)
    | PTarrow (t1, t2) -> PTarrow (map_ocaml_ty t1, map_ocaml_ty t2)
  ...
  let type_declaration ~ocaml ns t =
    ...
    let ty_lenses =
      List.map
        (fun x -> { x with locaml = map_ocaml_ty x.locaml })
        spec.ty_lenses
    in
  ...
  let val_des des spec ns =
    ...
    let lifts l =
      List.filter_map
        (fun x -> Option.map (fun x -> x.arg_pred) x.arg_spatial)
        l
    in
    let triple_pre = lifts args @ pre in
    let triple_post = lifts (update_args @ rets) @ post in
    let mk_updates arg =
      Option.bind arg.arg_spatial (fun x ->
          if x.ro then None else Some x.arg_log)
    in
