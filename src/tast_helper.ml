...
let mk_val_spec args ret pre checks post xpost dv pure equiv txt loc =
  ...
  List.iter (t_ty_check ty_prop) pre;
  List.iter (t_ty_check ty_prop) checks;
  List.iter (t_ty_check ty_prop) post;
...
let mk_fun_spec req ens var coer =
  ...
  List.iter (t_ty_check ty_prop) req;
  (* Check that postconditions are of type prop *)
  List.iter (t_ty_check ty_prop) ens;
...
let mk_function ?(result = None) ls r params def spec loc =
  ...
  Option.iter
    (fun spec ->
      ...
      List.iter (t_ty_check ty_prop) spec.fun_ens)
    spec;
