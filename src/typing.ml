...
let rec dterm whereami kid crcm ns denv { term_desc; term_loc = loc } : dterm =
  ...
  let apply dt1 t2 =
    ...
    let dty = dty_fresh () in
    unify dt1 (Tapp (ts_arrow, [ dt2.dt_dty; dty ]));
  ...
  | Uast.Tnot t ->
     ...
     dterm_unify dt (dty_of_ty ty_prop);
  ...
  | Uast.Tif (t1, t2, t3) ->
     ...
     let dt1 =
       try dterm_expected crcm dt1 dty_bool with
       |_ -> dterm_expected crcm dt1 (dty_of_ty ty_prop) in 
     let dty = max_dty crcm [ dt2; dt3 ] in
     let dt2 = dterm_expected crcm dt2 dty in
     let dt3 = dterm_expected crcm dt3 dty in
  ...
  | Uast.Tlet (pid, t1, t2) ->
     let dt1 = dterm whereami kid crcm ns denv  t1 in
     let denv = denv_add_var denv pid.pid_str (dt1.dt_dty) in
     let dt2 = dterm whereami kid crcm ns denv t2 in
  ...
  | Uast.Tinfix (t1, op1, t23) ->
     let apply de1 op de2 =
       ...
       (if ls_equal ls ps_equ then
          ...
          try
            dty_unify ~loc max (List.hd dtyl)
          with Exit -> ());
       let dtl =
         app_unify_map ~loc ls (dterm_expected crcm) [ de1; de2 ] dtyl
       in
       if op.pid_str = neq.id_str then
         mk_dterm ~loc (DTnot (mk_dterm ~loc (DTapp (ls, dtl)) dty)) (dty_of_ty ty_prop)
     ...
     let rec chain _ de1 op1 t23 =
       ...
       | { term_desc = Uast.Tinfix (t2, op2, t3); term_loc = loc23 } ->
          ...
          dfmla_unify de12;
          dfmla_unify de23;
          mk_dterm ~loc (DTbinop (Tand, de12, de23)) (dty_of_ty ty_prop)
  ...
  | Uast.Tbinop (t1, op, t2) ->
     ...
     dfmla_unify dt1;
     dfmla_unify dt2;
     mk_dterm ~loc (DTbinop (binop op, dt1, dt2)) (dty_of_ty ty_prop)
  | Uast.Tquant (q, vl, t) ->
     let get_dty pty =
     ...
     let dty, q =
       ...
       | Uast.Tforall ->
          dfmla_unify dt;
          (dty_of_ty ty_prop, Tforall)
       | Uast.Texists ->
          dfmla_unify dt;
          (dty_of_ty ty_prop, Texists)
     in
  ...
  | Uast.Tlambda (pl, t, pty) ->
     ...
     let denv =
       List.fold_left
         (fun denv (dp, _) -> env_union choose_snd denv dp.dp_vars)
         denv args
     in
     let dt = dterm whereami kid crcm ns denv  t in
     let dt =
       ...
       | _ -> dt
     in
     let dt_dty = dt.dt_dty in
     let dty =
       let apply (_, dty1) dty2 = Dterm.Tapp (ts_arrow, [ dty1; dty2 ]) in
       List.fold_right apply args dt_dty
     in
  ...
  | Uast.Tcase (t, ptl) ->
     ...
     let branch (p, g, t) =
       ...
       let denv = env_union choose_snd denv dp.dp_vars in
       let dt = dterm whereami kid crcm ns denv  t in
       let dg =
         match g with
         | None -> None
  ...
  | Uast.Told t -> (
    ...
    | _ ->
       let dt = dterm whereami kid crcm ns {denv with env=denv.old_env}t in
...
let process_type_spec kid crcm ns ty spec =
  ...
  in
  let invariants =
    List.map (fmla Invariant kid crcm ns {env; old_env=env}) (snd spec.ty_invariant)
  in
...
let type_type_declaration path kid crcm ns r tdl =
  ...
  and visit ~alias s td =
    ...
    let td_ts = mk_ts (Ident.create ~loc:td.tname.loc ~path s)
...
let process_val_spec kid crcm ns id args ret vs =
  ...
  let cs =  List.map (type_spatial Consumes env) vs.sp_consumes in
  let pres = List.map (type_spatial Preserves env) vs.sp_preserves in
  ...
  let prod_env, ret =
    ...
    | _, _ ->
       process_args `Return header.sp_hd_ret [ (ret, Asttypes.Nolabel) ] env.env [] in 
  
  let prod = List.map (type_spatial Produces {env=prod_env;old_env=Mstr.empty}) vs.sp_produces in
  
  let equal_arg arg (t, _) =
    Option.fold ~some:(fun arg ->
        match t.t_node with
        |Tvar v -> v.vs_name.id_str = arg.vs_name.id_str
        |Tapp (v, _) -> v.ls_name.id_str = arg.vs_name.id_str
        |_ -> false) ~none:false arg in 
  let term_of_vs x = 
    {t_node =Tvar x;
     t_ty = x.vs_ty;
     t_attrs=[];
     t_loc= Location.none} in
  
  let read_only = 
    let pres_args =
      List.filter
        (fun (_, x) -> not (List.exists (equal_arg x) (wr@cs@prod@pres))) args in
    pres @
      (List.filter_map (fun (_, x) ->
           Option.map (fun x -> term_of_vs x, x.vs_ty) x) pres_args) in 
  ...
  let spec_args = List.map (apply_spatial prod false) spec_args in 
  let spec_ret = List.map (apply_spatial prod false) spec_ret in

  let update_env env arg ty =
    match ty with
    |None -> env
    |Some (_, ty) ->
      let v = Option.get arg.arg_vs in
      let log_v = {v with vs_ty = ty} in 
      Mstr.add v.vs_name.id_str log_v env in
  ...
  let pre = List.map (fmla Requires kid crcm ns {env=old_env; old_env}) vs.sp_pre in
  ...
  mk_val_spec spec_args spec_ret pre checks post xpost vs.sp_diverge vs.sp_pure  vs.sp_equiv vs.sp_text vs.sp_loc
...
let process_val path ~loc ?(ghost = Nonghost) kid crcm ns vd =
  ...
  let () =
    ...
    if Ttypes.(ty_equal ret ty_unit) && args <> [] then
      ...
      | None -> ()
      | Some sp ->
         if List.for_all (fun x -> x.read_only) sp.sp_args then
...
let process_function path kid crcm ns f =
  let f_ty =
    Option.fold ~some:(ty_of_pty ns)
      ~none:ty_prop f.fun_type in
...
let process_axiom path loc kid crcm ns a =
  ...
  let t = fmla Axiom kid crcm ns {env=Mstr.empty;old_env= Mstr.empty} a.Uast.ax_term in
