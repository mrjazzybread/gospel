...
let process_val_spec kid crcm ns id args ret vs =
  ...
  let apply_spatial l vs =
    ...
    | Some (t, s_ty) ->
        let ty, s_ty =
          ...
          | _ -> assert false
        in
        Some (s_ty, ty_apply_spatial ty s_ty)
  in

  let apply_spatial_spec l con sa =
    match apply_spatial l sa.lb_vs with
    | None -> sa
    | Some sp ->
  ...
  let spec_args = List.map (apply_spatial_spec cs true) spec_args in
  let spec_args = List.map (apply_spatial_spec prod false) spec_args in
  let spec_ret = List.map (apply_spatial_spec prod false) spec_ret in
  ...
  let xspec = List.map process_xspec vs_post.sp_xspec in
  ...
  mk_val_spec spec_args spec_ret pre checks post xspec vs_pre.sp_diverge
