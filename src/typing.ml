...
let type_vars ~bind env pty =
  let rec type_vars = function
    | ParseUast.PTtyvar v when Hashtbl.mem env.type_vars v.Preid.pid_str ->
...
let unique_pty ~ocaml ~bind defs env pty =
  let rec unique_pty = function
    | ParseUast.PTtyvar pid -> PTtyvar (Hashtbl.find env.type_vars pid.pid_str)
...
let rec unique_post_term defs old_env env t =
  ...
  let term_desc =
    ...
    | Tscope (q, t) ->
        ...
        Tscope (q, t)
    | Told t ->
        let env = match old_env with None -> assert false | Some env -> env in
...
let fun_spec defs env spec =
  ...
  {
    fun_req = List.map unique_term spec.ParseUast.fun_req;
...
let tdecl_env (l : ParseUast.s_type_declaration list) =
  let open ParseUast in
  let add_decl env t =
...
let type_kind ~ocaml env tid tparams lenv = function
  | ParseUast.PTtype_abstract -> (env, PTtype_abstract)
  | PTtype_record l ->
      let open ParseUast in
      ...
      let fields_id =
        List.map (fun l -> (l.IdUast.pld_name, l.pld_type)) fields
      in
...
let type_decl ~ocaml lenv env t =
  ...
  let tname =
    try Env.find t.ParseUast.tname.pid_str lenv.type_nms
  ...
  let def_gen =
    ...
    fun env ->
      let tspec =
        Option.fold ~none:Tast2.empty_tspec
          ~some:(unique_tspec (scope env) model lenv self_ty inv_ty)
          t.tspec
      in
      Tast2.mk_tdecl tname tparams tkind t.tprivate tmanifest t.tattributes
        tspec t.tloc
  in
...
let tdecl_list ~ocaml env l =
  ...
  let defs_og_order =
    List.map
      (fun pdecl ->
        List.find
          (fun id_decl -> pdecl.tname.pid_str = id_decl.Tast2.tname.id_str)
          defs)
      l
  in
  (env, defs_og_order)
...
let process_exception exn defs =
  let exn_id = Ident.from_preid exn.ParseUast.exn_id in
...
let rec header_length = function
  | [] -> 0
  | x :: t -> (
      header_length t + match x with ParseUast.Lghost _ -> 0 | _ -> 1)
...
let valid_header loc header val_nm types rets =
  ...
  let header_nm = header.ParseUast.sp_hd_nm in
  ...
  let () =
    Utils.duplicate Uast_utils.v_eq dup_var_error
      (header.ParseUast.sp_hd_ret @ header.sp_hd_args)
  in
  (* Check if the number of arguments and return values is congruent
     with the value's type *)
  let expectedl, argl =
    (List.length types, header_length header.ParseUast.sp_hd_args)
  in
...
let rec pair_hd_vars types vars =
  ...
  | ty :: types, id :: ids -> (
      ...
      | Lghost _ ->
          (* Since ghost values are ignored, we call [pair_hd_vars]
             with the same [types] list. *)
          pair_hd_vars (ty :: types) ids)
...
let resolve_vars vars dup_error l =
  ...
  let error qid =
    let loc = (Structure.leaf qid).id_loc in
    W.error ~loc (dup_error (Uast_utils.flatten_ident qid))
  in
  let () =
    Utils.duplicate
      (fun x y -> Ident.equal (Structure.leaf x) (Structure.leaf y))
      error l
  in
  l
...
let duplicated_ocaml_var l1 l2 error =
  let qid_eq q1 q2 = Uast_utils.eq_qualid q1 q2 in
  List.iter
    (fun x ->
      if List.exists (qid_eq x) l1 then
        W.error ~loc:(Structure.leaf x).id_loc
...
let process_sugar_ownership spec vars =
  ...
  let dup_mod qid = W.Duplicated_modifies qid in
  let dup_pres qid = W.Duplicated_preserves qid in
  let pres_mod qid = W.Modified_and_preserved qid in
  (* Name resolution for names in [modifies] and [preserves] clauses.
     Also checks if there are any duplicates. *)
  let modified_vars = resolve_vars vars dup_mod spec.ParseUast.sp_modifies in
  let preserved_vars = resolve_vars vars dup_pres spec.ParseUast.sp_preserves in
...
let add_values_env header lenv defs args rets consumes produces read_only =
  ...
  let process_var pre_env post_env vals = function
    | ParseUast.Lwild | Lunit _ -> (Unnamed, pre_env, post_env)
    | Lghost (pid, pty) ->
        let id = Ident.from_preid pid in
    ...
    | Lvar pid ->
        ...
        let var_mem = function Qid id -> var_mem id | _ -> false in
  ...
  let args, pre_env, post_env =
    process_header lenv lenv args header.ParseUast.sp_hd_args
  in
  let rets, pre_env, post_env =
    process_header pre_env post_env rets header.ParseUast.sp_hd_ret
  in
...
let insert_ownership consumes produces ro args rets =
  (* [is_own ~ret q1] checks if [q1] is in any ownership clause.  If
     the [ret] flag is [true], this function only checks the
     [produces] clauses.  If so, this function turns the identifier
     into a [qualid]. *)
  let is_own (q1, _) =
    (* We turn [q1] to a [qualid]. *)
    let q1 = Qid q1 in
    (* Checks if [q2] is equal to [q1] *)
    let qid_eq q2 = Uast_utils.eq_qualid q1 q2 in
    (* Check if [q1] is in either the [produces] or [consumes]. *)
    if List.exists qid_eq produces || List.exists qid_eq consumes then None
    else Some q1
  in
...
let type_val_spec spec defs args rets pre_env post_env =
  let pre = List.map (unique_term defs pre_env) spec.ParseUast.sp_pre in
  let post = List.map (unique_term defs post_env) spec.sp_post in
...
(** [ocaml_value env v] Translates the specification of an OCaml value
    ...
    - Typechecking the pre and post conditions and creating the typed value
      description. *)
let ocaml_value env v =
  let defs = scope env in
  (* From the function's type, get the types of the values' arguments *)
  let args, rets = Uast_utils.args_to_list v.ParseUast.vtype in
  (* Create a header if the user did not provide one. *)
  let header =
    Option.fold
      ~none:(Uast_utils.create_header v.vname (List.length args))
      ~some:(fun header ->
        (* If the user has provided a header, then we check if there
             are any errors. *)
        valid_header v.vloc header v.vname args rets;
        header)
      v.vspec.sp_header
  in
  let lenv = empty_local_env () in
  (* Function to perform name resolution on non-ghost function types. *)
  let unique_ocaml_pty = unique_pty ~ocaml:true ~bind:true defs lenv in
  let arg_types = List.map unique_ocaml_pty args in
  let ret_types = List.map unique_ocaml_pty rets in
  (* If the function returns a tuple but the user only supplied one
     ...
     list with a tuple. *)
  let ret_types =
    if List.length ret_types > 1 && List.length header.sp_hd_ret = 1 then
      [ PTtuple ret_types ]
    else ret_types
  in
  ...
  (* Resolves the variables in [modifies] and [preserves] clauses.
     ...
     [preserves] clauses. *)
  let mod_and_pres, read_only = process_sugar_ownership v.vspec args in
  (* Resolves the variables in [consumes] and [produces] clauses.
     Note how return values are not allowed to be consumed. *)
  let consumes =
    process_ownership ~consumes:true mod_and_pres v.vspec.sp_consumes args
  in
  let produces =
    process_ownership ~consumes:false mod_and_pres v.vspec.sp_produces
      (args @ rets)
  in
  ...
  in
  let pre, post = type_val_spec v.vspec defs args rets pre_env post_env in
  let vspec =
    {
      Tast2.sp_args = args;
      sp_rets = rets;
      sp_pre = pre;
      sp_post = post;
      sp_diverge = v.vspec.sp_diverge;
      sp_pure = v.vspec.sp_pure;
      sp_text = v.vspec.sp_text;
      sp_loc = v.vspec.sp_loc;
    }
  in
  {
    Tast2.vname = Ident.from_preid v.vname;
    vtype = unique_ocaml_pty v.vtype;
    vprim = v.vprim;
...
and signature s env =
  let sdesc, env =
    ...
    | Sig_val v -> (Tast2.Sig_value (ocaml_value env v), env)
    | Sig_type t ->
        let env, t = tdecl_list ~ocaml:true env t in
        (Tast2.Sig_ghost_type t, env)
