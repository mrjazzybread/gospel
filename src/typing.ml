...
let rec q_loc = function Qid pid -> pid.pid_loc | Qdot (q, _) -> q_loc q
...
let rec dpattern kid ns { pat_desc; pat_loc = loc } =
  ...
  | Papp (q, ([ { pat_desc = Prec defined; _ } ] as pl)) -> (
      ...
      | Constructor_symbol { ls_name; ls_args = Cstr_record expected; ls_value }
        as ls -> (
          let get_pid_str = function Qid pid | Qdot (_, pid) -> pid.pid_str
          and get_pid_loc = function Qid pid | Qdot (_, pid) -> pid.pid_loc
          and constr_str =
...
let rec dterm whereami kid crcm ns denv ({ term_desc; term_loc = loc } as t) :
  ...
  let qualid_app q tl =
    match q with
    | Qid ({ pid_loc = loc; pid_str = s; _ } as pid) -> (
  ...
  | Ttrue -> mk_dterm ~loc DTtrue dty_bool
  ...
  | Tvar (Qid pid) when is_in_denv denv pid.pid_str ->
  ...
  | Tapply
      ...
      | Field_symbol { ls_name; _ } ->
          W.error ~loc (W.Field_application ls_name.id_str))
  | Tapply (({ term_desc = Tvar q; _ } as t1), t2) -> (
      try
  ...
  | Ttuple [] -> fun_app ~loc fs_unit []
  | Ttuple tl -> fun_app ~loc (fs_tuple (List.length tl)) tl
  | Tlet (pid, t1, t2) ->
      let dt1 = dterm whereami kid crcm ns denv t1 in
      let denv = denv_add_var denv pid.pid_str (dty_of_dterm dt1) in
      let dt2 = dterm whereami kid crcm ns denv t2 in
      mk_dterm ~loc (DTlet (pid, dt1, dt2)) (Option.get dt2.dt_dty)
  | Tinfix _ -> dterm whereami kid crcm ns denv (Uast_utils.chain t)
  | Tquant (q, vl, t) ->
      let get_dty pty =
...
let process_val_spec kid crcm ns id args ret vs =
  ...
  let process_args where args tyl env lal =
    ...
    let rec aux args tyl env lal =
      ...
      | ParseUast.Lghost (pid, pty) :: args, _ ->
...
let mk_dummy_var i (ty, arg) =
  ...
  match arg with
  | _ when ty_equal ty ty_unit -> ParseUast.Lunit
  | Nolabel -> Lnone (Preid.create ~loc ("__arg" ^ string_of_int i))
  | Labelled s -> Lnamed (Preid.create ~loc s)
  | Optional s -> Loptional (Preid.create ~loc s)
...
let process_axiom path loc kid crcm ns a =
  let id = Ident.of_preid ~path a.ParseUast.ax_name in
  let t = fmla Axiom kid crcm ns Mstr.empty a.ax_term in
...
and process_sig_item path penv muc { sdesc; sloc } =
  let process_sig_item si muc =
    ...
    match si with
    | ParseUast.Sig_type (r, tdl) ->
        (muc, process_sig_type path ~loc:sloc kid crcm ns r tdl)
    | Sig_val vd -> (muc, process_val path ~loc:sloc kid crcm ns vd)
    | Sig_typext te -> (muc, mk_sig_item (Sig_typext te) sloc)
    | Sig_module m -> process_mod path penv sloc m muc
    | Sig_recmodule _ -> W.error ~loc:sloc (W.Unsupported "module rec")
    | Sig_modsubst _ | Sig_modtypesubst _ | Sig_typesubst _ ->
        W.error ~loc:sloc (W.Unsupported "type substitution")
    | Sig_modtype mty_decl -> process_modtype_decl path penv sloc mty_decl muc
    | Sig_exception te -> (muc, process_exception_sig path sloc ns te)
    | Sig_open od -> process_open ~loc:sloc ~ghost:Nonghost penv muc od
    | Sig_include id -> (muc, mk_sig_item (Sig_include id) sloc)
    | Sig_class cdl -> (muc, mk_sig_item (Sig_class cdl) sloc)
    | Sig_class_type ctdl -> (muc, mk_sig_item (Sig_class_type ctdl) sloc)
    | Sig_attribute a -> (muc, mk_sig_item (Sig_attribute a) sloc)
    | Sig_extension (e, a) -> (muc, mk_sig_item (Sig_extension (e, a)) sloc)
    | Sig_function f -> (muc, process_function path kid crcm ns f)
    | Sig_axiom a -> (muc, process_axiom path sloc kid crcm ns a)
    | Sig_ghost_type (r, tdl) ->
        (muc, process_sig_type path ~loc:sloc ~ghost:Ghost kid crcm ns r tdl)
    | Sig_ghost_val vd ->
        (muc, process_val path ~loc:sloc ~ghost:Ghost kid crcm ns vd)
    | Sig_ghost_open od -> process_open ~loc:sloc ~ghost:Ghost penv muc od
  in
