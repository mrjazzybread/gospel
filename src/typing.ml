...
let ns_find ~loc f ns sl =
  match sl with
  | s :: _ :: _ when not (ns_exists_ns ns s) -> raise (Ns_not_found (loc, s))
...
let rec dterm whereami kid crcm ns denv { term_desc; term_loc = loc } : dterm =
  ...
  | Uast.Tif (t1, t2, t3) ->
      ...
      let dt1 =
        try dterm_expected crcm dt1 dty_bool
        with _ -> dterm_expected crcm dt1 (dty_of_ty ty_prop)
      in
  ...
  | Uast.Tinfix (t1, op1, t23) ->
      let apply de1 op de2 =
        ...
        (if ls_equal ls ps_equ then
           ...
           try dty_unify ~loc max (List.hd dtyl) with Exit -> ());
        let dtl =
  ...
  | Uast.Told t -> (
      ...
      | _ ->
          let dt =
            dterm whereami kid crcm ns { denv with env = denv.old_env } t
          in
...
let process_type_spec kid crcm ns ty spec =
  ...
  let invariants =
    List.map
      (fmla Invariant kid crcm ns { env; old_env = env })
      (snd spec.ty_invariant)
  in
...
let type_type_declaration path kid crcm ns r tdl =
  ...
  and visit ~alias s td =
    ...
    let spatial =
      Option.fold ~some:(fun (t, b) -> Model (t, b)) ~none:Self td_model
    in
    let td_ts =
      mk_ts
    ...
    let model_ty = match td_model with Some (ty, _) -> ty | None -> ty in
...
let process_val_spec kid crcm ns id args ret vs =
  ...
  let type_spatial whereami env t =
    ...
    let spatial_type =
      match t.Uast.s_type with None -> ty | Some t -> ty_of_pty spatial_ns t
    in
    (typed, spatial_type)
  in
      ...
      | Some (p, t) ->
          ...
          let ty =
            let rec aux p xs =
              ...
              | ( (Pvar _ | Pwild | Ptuple _ | Pconst _ | Pinterval (_, _)),
                  Exn_tuple [ ty ] ) ->
                  ty
