...
and term_desc =
  ...
  (* [Tinfix] represents a chain of infix operations such as
     [e1 < e2 < ...]. This node is necessary during parsing so that
     there exists a distinction between of chains of non-associative
     infix operators such as (x < y < z) and (x < (y < z)). During
     typing, this node is desugared into a normal function application
     and handled the same way as [Tapply] *)
  | Tnot of term
  (* TODO remove not *)
