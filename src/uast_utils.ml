...
(** [mk_op_apply op arg1 arg2] Creates a term for the application of the
    operator [op] to [arg1] and [arg2]. Although this function could be used for
    any function application, the locations of the subterms would not correspond
    to what would be expected. *)
let mk_op_apply op l =
  let f = mk_term (Tpreid (Qpreid op)) op.pid_loc in
  let t =
    List.fold_left (fun f arg -> mk_term (Tapply (f, arg)) arg.term_loc) f l
  in
  t.term_desc
...
let rec chain t =
  ...
  | Tinfix (t1, o, t2) -> (
      ...
      (* We match on the right subtree due to the associativity of
         infix operators*)
      | Tinfix (t3, _, _) ->
          (* If we reach this case, then the term [t] is [t1 < t2]
            where [t2] = [t3 <= ...] and [<] and [<=] stand for an
            arbitrary infix operator. In this case, we create the
            conjunction [t1 < t3 /\ t2] *)
          let t = chain t2 in
