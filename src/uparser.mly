...
%{
  ...
  let mk_pid pid ?(style = Normal) l = Preid.create pid ~style ~attrs:[] ~loc:(mk_loc l)
  let mk_term d l = { term_desc = d; term_loc = mk_loc l }

  let get_op l = mk_pid ~style:Preid.Mixfix "[_]" l
  let set_op l = mk_pid ~style:Preid.Mixfix "[->]" l
  let sub_op l = mk_pid ~style:Preid.Mixfix "[_.._]" l
  let above_op l = mk_pid ~style:Preid.Mixfix "[_..]" l
  let below_op l = mk_pid ~style:Preid.Mixfix "[.._]" l
...
| l = term ; COLONCOLON ; r = term
    { mk_op_apply (mk_pid ~style:Mixfix "::" $loc) [l; r] }
...
| LEFTBRCRIGHTBRC
    { Tvar (Qid (mk_pid ~style:Mixfix ( "{}") $loc)) }
| LEFTBRCCOLON t=term COLONRIGHTBRC
    { let id = mk_pid ~style:Mixfix ("{:_:}") $loc in
      mk_op_apply id [t] }
;
...
%inline oppref:
| o = OPPREF { mk_pid ~style:Prefix o $loc }
;

prefix_op:
| o = op_symbol { mk_pid ~style:Prefix o $loc }
;

%inline infix_op_1:
| o = OP1   { mk_pid ~style:Infix o $loc }
| EQUAL     { mk_pid ~style:Infix "=" $loc }
| LTGT      { mk_pid ~style:Infix "<>" $loc }
%inline infix_op_234:
| o = OP2   { mk_pid ~style:Infix o $loc }
| o = OP3   { mk_pid ~style:Infix o $loc }
| STAR      { mk_pid ~style:Infix "*" $loc }
| o = OP4   { mk_pid ~style:Infix o $loc }
| ARROW     { mk_pid ~style:Infix "->"  $loc }
| LRARROW   { mk_pid ~style:Infix "<->" $loc }
| DISJ      { mk_pid ~style:Infix "\\/" $loc }
| BARBAR    { mk_pid ~style:Infix "||"  $loc }
| CONJ      { mk_pid ~style:Infix "/\\" $loc }
| AMPAMP    { mk_pid ~style:Infix "&&"  $loc }
;
...
lident_op_id:
| id = lident_fun_id               { id }
| LEFTPAR DISJ RIGHTPAR            { mk_pid ~style:Infix "\\/" $loc }
| LEFTPAR CONJ RIGHTPAR            { mk_pid  ~style:Infix "/\\" $loc }
;
