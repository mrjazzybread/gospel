...
Module Declarations.
  ...
  Class __mod_sig  := { _mod : integer -> integer -> integer }.
  ...
  Module Sequence.
    ...
    `{@_empty_sig _}
     := {
      empty_length :
        forall {A}, forall `{Inhabited A}, (length (A := A) empty) = 0
    }.
    ...
    Class _hd_def_sig `{@_sequence_sig} `{@_hd_sig _} `{@_seq_get_sig _}  := {
      hd_def :
    ...
    `{@_empty_sig _}
     := {
      mult_empty :
        forall {A}, forall `{Inhabited A}, forall (x : A), (multiplicity x empty)
        =
        0
    }.
    ...
    `{@_mem_sig _}
     := {
      mem_fun_def :
        forall {A}, forall `{Inhabited A}, forall (x : A) (s : sequence A), (belongs x s)
        <->
        (mem x s)
    }.
    ...
    `{@_belongs_sig _}
     := {
      nmem_def :
        forall {A}, forall `{Inhabited A}, forall (s : sequence A) (x : A), (neg_belongs x s)
        <->
        (not (belongs x s))
    }.
    ...
    Class _get_def_sig `{@_sequence_sig} `{@_get_sig _} `{@_seq_get_sig _}  := {
      get_def :
        forall {A}, forall `{Inhabited A}, forall
        (s : sequence A)
        (i : integer),
    ...
    `{@_set_sig _}
     := {
      set_elem_other :
        ...
        (in_range s i2)
        ->
        ((seq_get (set s i1 x) i2) = (seq_get s i2))
    }.
  ...
  Module Bag.
    ...
    Class _well_formed_sig `{@_bag_sig} `{@_multiplicity_sig _}  := {
      well_formed :
        forall {A}, forall `{Inhabited A}, forall (b : bag A) (x : A), (multiplicity x b)
        >=
        0
    }.
    ...
    Class _mem_fun_def_sig `{@_bag_sig} `{@_belongs_sig _} `{@_mem_sig _}  := {
      mem_fun_def :
        forall {A}, forall `{Inhabited A}, forall (x : A) (s : bag A), (belongs x s)
        <->
        (mem x s)
    }.
    ...
    `{@_belongs_sig _}
     := {
      nmem_def :
        forall {A}, forall `{Inhabited A}, forall (s : bag A) (x : A), (neg_belongs x s)
        <->
        (not (belongs x s))
    }.
    ...
    `{@_multiplicity_sig _}
     := {
      mem_def :
        forall {A}, forall `{Inhabited A}, forall (x : A) (b : bag A), (belongs x b)
        <->
        ((multiplicity x b) > 0)
    }.
    ...
    `{@_add_sig _}
     := {
      add_mult_x :
        forall {A}, forall `{Inhabited A}, forall (b : bag A) (x : A), (multiplicity x (add x b))
        =
        (1 + (multiplicity x b))
    }.
    ...
    `{@_singleton_sig _}
     := {
      singleton_fun_def :
        forall {A}, forall `{Inhabited A}, forall (x : A), (singleton_set x)
        =
        (singleton x)
    }.
    ...
    `{@_max_sig}
     := {
      remove_mult_x :
        forall {A}, forall `{Inhabited A}, forall (b : bag A) (x : A), (multiplicity x (remove x b))
        =
        (max 0 ((multiplicity x b) - 1))
    }.
    ...
    Class _sum_all_sig `{@_bag_sig} `{@_multiplicity_sig _} `{@_sum_sig _}  := {
      sum_all :
        forall {A}, forall `{Inhabited A}, forall
        (b : bag A)
        (b' : bag A)
    ...
    `{@_neg_belongs_sig _}
     := {
      disjoint_def :
        ...
        <->
        (forall (x : A), (belongs x b) -> (neg_belongs x b'))
    }.
    ...
    `{@_multiplicity_sig _}
     := {
      subset_def :
        forall {A}, forall `{Inhabited A}, forall (b : bag A) (b' : bag A), (subset b b')
        <->
        (forall (x : A), (multiplicity x b) <= (multiplicity x b'))
    }.
    ...
    `{@Sequence._belongs_sig _}
     := {
      finite_def :
        ...
        <->
        (exists (s : sequence A), forall (x : A), (belongs x b)
        ->
        (Sequence.belongs x s))
    }.
    ...
    Class _card_empty_sig `{@_bag_sig} `{@_cardinal_sig _} `{@_empty_sig _}  := {
      card_empty :
        forall {A}, forall `{Inhabited A}, (cardinal (A := A) empty) = 0
    }.
    ...
    `{@_singleton_set_sig _}
     := {
      card_singleton :
        forall {A}, forall `{Inhabited A}, forall (x : A), (cardinal (singleton_set x))
        =
        1
    }.
    ...
    `{@_of_seq_sig _ _}
     := {
      of_seq_multiplicity :
        forall {A}, forall `{Inhabited A}, forall (s : sequence A) (x : A), (Sequence.multiplicity x s)
        =
  ...
  Module _Set.
    ...
    Class _mem_fun_def_sig `{@_set_sig} `{@_belongs_sig _} `{@_mem_sig _}  := {
      mem_fun_def :
        forall {A}, forall `{Inhabited A}, forall (x : A) (s : set A), (belongs x s)
        <->
        (mem x s)
    }.
    ...
    `{@_neg_belongs_sig _}
     := {
      nmem_def :
        forall {A}, forall `{Inhabited A}, forall (s : set A) (x : A), (not (belongs x s))
        <->
        (neg_belongs x s)
    }.
    ...
    `{@_empty_sig _}
     := {
      empty_mem :
        forall {A}, forall `{Inhabited A}, forall (x : A), neg_belongs x empty
    }.
    ...
    Class _add_mem_sig `{@_set_sig} `{@_belongs_sig _} `{@_add_sig _}  := {
      add_mem :
        forall {A}, forall `{Inhabited A}, forall (s : set A) (x : A), belongs x (add x s)
    }.

    Class _add_mem_neq_sig `{@_set_sig} `{@_belongs_sig _} `{@_add_sig _}  := {
      add_mem_neq :
        forall {A}, forall `{Inhabited A}, forall (s : set A) (x : A) (y : A), (x
        <>
        y)
        ->
        ((belongs x s) <-> (belongs x (add y s)))
    }.
    ...
    `{@_remove_sig _}
     := {
      remove_mem :
        forall {A}, forall `{Inhabited A}, forall (s : set A) (x : A), neg_belongs x (remove x s)
    }.
    ...
    `{@_remove_sig _}
     := {
      remove_mem_neq :
        ...
        y)
        ->
        ((belongs x s) <-> (belongs x (remove y s)))
    }.
    ...
    Class _union_mem_sig `{@_set_sig} `{@_belongs_sig _} `{@_union_sig _}  := {
      union_mem :
        forall {A}, forall `{Inhabited A}, forall
        (s : set A)
        (s' : set A)
    ...
    Class _inter_mem_sig `{@_set_sig} `{@_belongs_sig _} `{@_inter_sig _}  := {
      inter_mem :
        forall {A}, forall `{Inhabited A}, forall
        (s : set A)
        (s' : set A)
    ...
    Class _subset_def_sig `{@_set_sig} `{@_subset_sig _} `{@_belongs_sig _}  := {
      subset_def :
        forall {A}, forall `{Inhabited A}, forall (s : set A) (s' : set A), (subset s s')
        <->
        (forall (x : A), (belongs x s) -> (belongs x s'))
    }.
    ...
    Class _set_map_sig `{@_set_sig} `{@_belongs_sig _} `{@_map_sig _}  := {
      set_map :
    ...
    `{@Sequence._belongs_sig _}
     := {
      finite_def :
        ...
        <->
        (exists (seq : sequence A), forall (x : A), (belongs x s)
        ->
        (Sequence.belongs x seq))
    }.
