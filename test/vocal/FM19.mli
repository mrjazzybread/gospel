...
(*@ modifies q1, q2
    let _ = in_place_concat q1 q2 in
...
(*@ consumes q1
    modifies q2
    let _ = in_place_destructive_concat q1 q2 in
    ensures  q2.view = old q1.view ++ old q2.view *)

val nondestructive_concat : 'a t -> 'a t -> 'a t
(*@ let q3 = nondestructive_concat q1 q2 in
...
(*@ requires n >= 1
    let r, [k: integer] = power_2_below n in
...
(*@ requires Set.mem e1 uf.dom && Set.mem e2 uf.dom
    ...
    let b = equiv [uf: uf_instance] e1 e2 in
      ensures  b <-> uf.rep e1 = uf.rep e2 *)

(*@ val create_instance: unit -> uf_instance *)
(*@ let uf = create_instance () in
    ensures uf.dom = {} *)

val make : unit -> elem
(*@ modifies uf
    let e = make [uf: uf_instance] () in
...
(*@ requires true (* P in the paper *)
    ...
    let p5, p6, m, [h: integer] = f p1 p2 p3 p4 n [g: integer] in
